...
class BookingDetailController extends Controller
{
    ...
    public function bookings_on_tour()
    {
        ...
        foreach ($tours as $key => $tour) {
            $startDate = Location::find(1)->where(['tour_id' => $tour['id']])->first()['startDate'];
            $bookings = BookingDetail::all()->where('tour_id', $tour['id']);
            ...
            $data[$key]['tourName'] = $tour['name'];
            $data[$key]['startDate'] = $startDate;
            $data[$key]['maxGroupSize'] = $tour['maxGroupSize'];
            $data[$key]['numberOfPeopleBooked'] = $bookings->count();
            $data[$key]['approvedOrderQuantity'] = $approvedOrderQuantity;
